...
#include "lsched.h"
extern acoral_thread_t *acoral_thread_prev;
extern acoral_thread_t *acoral_thread_next;
...
///重定义普通切换上下文函数，为上层使用
#define HAL_COMM_SWITCH() do{acoral_switch_context();soc3883_context_switch(&(acoral_thread_prev->stack), &(acoral_thread_next->stack));}while(0)
