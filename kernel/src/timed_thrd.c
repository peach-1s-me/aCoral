 ...
 *         <tr><td>v2.0 <td>文佳源 <td>2025-04-15 <td>规范代码
 */
#include "type.h"
#include "cpu.h"
#include "queue.h"
...
#include "print.h"
#include "error.h"
#include "calculate_time.h" /* 定时器计时 */
#include "measure.h"        /* 开销测试 */

static acoral_sched_policy_t timed_policy;           /* 时间确定性线程策略结构体实例 */
static acoral_queue_t timed_time_queue[CFG_MAX_CPU]; /* 时间确定性线程队列 */
 ...
 */
static acoral_id timed_policy_thread_init(
...
)
{
    ...
    {
        /*  设置时间确定性线程数据 */
        acoral_timed_policy_data_t *policy_data = (acoral_timed_policy_data_t *)p_data;
        thread->cpu = policy_data->cpu;
        thread->prio = ACORAL_TIMED_PRIO;

        /*  给私有数据分配内存 */
        timed_private_data_t *private_data = 
            (timed_private_data_t *)acoral_vol_malloc(sizeof(timed_private_data_t));
        ...
        private_data->ipc           = (acoral_ipc_t **)acoral_vol_malloc(private_data->section_num *
                                                                         sizeof(acoral_ipc_t *));
...
static void timed_policy_thread_release(acoral_thread_t *thread)
{
    ...
    /*  回收私有数据内存 */
    acoral_vol_free(thread->private_data);
...
static void timed_time_deal()
{
    ...
    {
        acoral_list_t *head = &timed_time_queue[cpu].head;
        if (acoral_list_empty(head))
        {
            /*  最后的循环才return */
            if (CFG_MAX_CPU - 1 == cpu)
            {
                return;
            }
            else
        ...
        acoral_list_t *itor;
        for (itor = head->next; itor!= head;) /*  轮询找到要被唤醒的线程 */
        {
            if (itor->value > 0)
            {
                break;
            ...
            timed_private_data_t *private_data = NULL;
            acoral_thread_t *thread = list_entry(itor, acoral_thread_t, timing);
            ...
            /* 利用临时指针保存下一个节点并删除当前节点 */
            acoral_list_t *tmp = itor->next;
