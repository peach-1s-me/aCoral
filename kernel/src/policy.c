 ...
 *         <tr><td>v2.0 <td>饶洪江 <td>2025-04-03 <td>规范代码风格
 */
#include "type.h"
#include "hal.h"
#include "queue.h"
...
#endif
/* 策略队列 */
static acoral_queue_t policy_queue;
 ...
 */
static acoral_id acoral_policy_thread_init(acoral_u32       policy_type,
...
)
{
    ...
    if (NULL == policy_ctrl || NULL == policy_ctrl->policy_thread_init) /* 指针检测 */
    {
    ...
    /* 进入具体策略的线程初始化函数 */
    return policy_ctrl->policy_thread_init(thread, route, args, p_data, data);
}
 ...
 * @return acoral_id  线程id
 */
acoral_id create_thread_by_policy(void                (*route)(void *args),
                                  acoral_u32            stack_size,
                                  void                 *args,
                                  acoral_char          *name,
                                  void                 *stack,
                                  acoral_u32            policy_type,
                                  void                 *p_data, 
                                  void                 *data,
                                  acoral_thread_hook_t *hook
)
{
#ifdef CFG_TRACE_THREADS_SWITCH_ENABLE
    acoral_id ret_id;
...
#ifdef CFG_TRACE_THREADS_SWITCH_ENABLE
    /* 基于策略的线程初始化函数 */
    ret_id = acoral_policy_thread_init(policy_type, thread, route, args, p_data, data);
    acoral_tinfos_add(thread);
    return ret_id;
#else
    /* 基于策略的线程初始化函数 */
    return acoral_policy_thread_init(policy_type, thread, route, args, p_data, data);
#endif
}
...
#endif
    timed_policy_init();                   /* 时间确定性线程策略初始化 */
}
